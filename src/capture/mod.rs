...
use crate::pipeline;
use crate::types::{Templates, UiMap};
...
impl CaptureSession {
    ...
    pub fn capture_frame(&mut self) -> Result<Option<DynamicImage>> {
        ...
        let res = unsafe { self.duplication.AcquireNextFrame(0, &mut frame_info, &mut desktop_resource) };
        if res.is_err() {
            return Ok(None);
        }
        ...
        let image = unsafe {
            if self.context.Map(&self.staging_texture, 0, D3D11_MAP_READ, 0, Some(&mut mapped)).is_ok() {
...
fn print_telemetry(results: &crate::types::Results, duration: Duration) {
    ...
    println!("[Telemetry] {} | Time: {}ms", parts.join(" | "), duration.as_millis());
}
...
mod tests {
    ...
    fn test_bgra_to_rgb_conversion() {
        ...
        buffer[12] = 0; buffer[13] = 0; buffer[14] = 255; buffer[15] = 255;
        // Row 1, Col 1: White (B=255, G=255, R=255, A=255)
        buffer[16] = 255; buffer[17] = 255; buffer[18] = 255; buffer[19] = 255;
