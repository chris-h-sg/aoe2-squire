...
fn test_live_screen_capture() -> Result<()> {
    ...
    while attempts < 10 {
        let res =
            unsafe { duplication.AcquireNextFrame(100, &mut frame_info, &mut desktop_resource) };
        if res.is_ok() {
            success = true;
            break;
        }
    ...
    assert!(
        success,
        "Failed to acquire a frame after 10 attempts. Make sure the screen is updating."
    );
